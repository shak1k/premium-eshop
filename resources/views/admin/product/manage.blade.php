                            ...
                            @foreach($products as $product)
                                <tr>
                                    ...
                                    <td class="d-flex">
                                        ...
                                        <a href="{{route('product.update-status', ['id' => $product->id])}}" class="btn {{$product->featured_status == 1 ? 'btn-primary' : 'btn-warning'}} btn-sm me-1">
                                            <i class="fa fa-arrow-circle-up"></i>
                                        </a>
                                        <form action="{{route('product.destroy', $product->id)}}" method="POST">
                                            @method('DELETE')
                                            @csrf
                                            <button type="submit" onclick="return confirm('Are you sure you want to delete this?')" class="btn btn-danger btn-sm me-1">
